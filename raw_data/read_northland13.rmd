...
data_rpm <- data_rpm %>%
  ...
  select(date_grazed, block, cultivar, seed_rate, grazing, date_pre, mass_pre, date_post, mass_post) %>%   mutate(date_pre = if_else(is.na(date_pre), date_post - 9, date_pre)) # some dates are missing
...
data_cut <- data_cut2 %>%
  ...
  select(date, block, seed_rate, cultivar, grazing, yield, dm_pc) %>% 
  group_by(date, block, seed_rate, cultivar, grazing) %>%
  summarise(yield = mean(yield),
            dm_pc = mean(dm_pc)) 
...
temp <- data_cut %>%
  ...
  select(date, block, seed_rate, cultivar, yield.x, dm_pc, date_cut.x, grazing,
...
temp <- data_bot %>%
  ...
  select(leaf, stem, leaf_below, stem_below, ann, wc, og, weed, dead, date, total, block, seed_rate, cultivar, rgfrac) 

#
data_bot <- temp
  
# add grazing no (inefficient!)
data_graze <- data_cut %>% 
  # ungroup %>%  # note data_cut is grouped by block, cultivar, seed_rate
  distinct(block, cultivar, seed_rate, grazing, date_cut) 
data_bot$grazing <- NA_integer_
data_bot$date_grazed <- as.Date(NA)
data_bot$delay <- NA_integer_
for (i in 1:nrow(data_bot)){
  temp <- data_graze %>% 
    filter(block==data_bot$block[i] & cultivar==data_bot$cultivar[i] & seed_rate==data_bot$seed_rate[i])
  if (nrow(temp)>0){
    delay <- data_bot$date[i] - temp$date_cut
    j <- which(abs(delay)==min(abs(delay)))
    data_bot$grazing[i] <- temp$grazing[j]
    data_bot$date_grazed[i] <- temp$date_cut[j]
    data_bot$delay[i] <- delay[j]
  }
}
data_bot <- data_bot %>% 
  arrange(desc(abs(delay)))
unique(data_bot$delay) # check delay is not too big
stopifnot(FALSE)
...
# This chart is not showing all the data for some reason!
...
data_bm <- data_bot %>%
  ...
  left_join(data_cut, by=c('block', 'seed_rate', 'cultivar', 'grazing')) %>% # need grazing number!
  select(date_bot, date_cut, block, seed_rate, cultivar, leaf, stem, ann, wc, og, weed, dead, 
         yield, below, growth_rate_pre, leaf_below, stem_below) %>%
...
data_sm <- read_xlsx(infile_name, sheet='Soil moisture data')
...
aseed_rate <- '18kg'
calib_start <- ymd("20110101") # period for data weight = 1
calib_end <- ymd("20140531") # period for data weight = 1
ablock <- 3
...
data_c[[3]] <- with(temp, tibble(var='TILTOT', year=year(date), 
                                 doy=yday(date), data=mean_tillers, sd=400, type="sd",
                                 weight=ifelse(((date>=calib_start)&(date<=calib_end)), 1, 0)) %>% drop_na())

# ryegrass mass (total or above cutting height? depending on definition of yield_bot)
temp <- data_bm %>%
  rename(date = date_cut) %>%
  select(block, seed_rate, cultivar, leaf, stem, leaf_below, stem_below, yield_bot, below, date) %>%
  filter(block==ablock & seed_rate==aseed_rate & cultivar==acultivar) 
data_c[[4]] <- with(temp, tibble(var='CLV', year=year(date), 
                                 doy=yday(date), data=leaf/100*yield_bot/10*0.45+leaf_below/100*below/10*0.45, 
                                 sd=10, type="sd",
                                 weight=ifelse(((date>=calib_start)&(date<=calib_end)), 1, 0)) %>% drop_na())
data_c[[5]] <- with(temp, tibble(var='CST', year=year(date), 
                                 doy=yday(date), data=stem/100*yield_bot/10*0.45+stem_below/100*below/10*0.45, 
                                 sd=4, type="sd",
                                 weight=ifelse(((date>=calib_start)&(date<=calib_end)), 1, 0)) %>% drop_na())
...
#                                  doy=yday(date), data=mean_sm, sd=2, type="sd",
#                                  weight=1) %>% drop_na())
