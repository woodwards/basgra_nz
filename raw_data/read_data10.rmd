...
Average pre-graze growth rate = `r round(mean(data_rpm$growth_rate_pre, na.rm=TRUE))`
...
data_rpm <- data_rpm %>%
  ...
  mutate(
    ...
    mass_grazed = mass_pre + growth_rate_pre * delay_pre,
    mass_resid = mass_post - growth_rate_post * delay_post, # assumes growth starts immediately
    harv = (1 - mass_resid / mass_grazed) * 100 # proportion harvested
    )

```

Average pre-graze growth rate = `r round(mean(data_rpm$harv, na.rm=TRUE))`
...
data_bc <- data_rpm %>%
  ...
  drop_na() %>%
  mutate(
    mass_cut = mass_pre + growth_rate_pre * delay,
...
Average cut DM% = `r round(mean(data_cut$dm_pc, na.rm=TRUE))`
...
data_bm <- data_bot %>%
  ...
  mutate(
    ...
    yield_bot = yield_cut + below + growth_rate_pre * delay # assumed on date_bot, including below mass?
    )
