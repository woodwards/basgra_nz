...
# calibration period
calib_start <- ymd("20120101")
calib_end <- ymd("20150531")
...
data_c[[3]] <- with(temp, tibble(var='TILTOT', year=year(date), 
                                 doy=yday(date), data=mean_tillers, sd=200, type="sd",
...
data_c[[4]] <- with(temp, tibble(var='CLV', year=year(date), 
                                 doy=yday(date), data=leaf/100*yield_bot/10*0.45, sd=5, type="sd",
                                 weight=ifelse(((date>=calib_start)&(date<=calib_end)), 1, 0)) %>% drop_na())
data_c[[5]] <- with(temp, tibble(var='CST', year=year(date), 
                                 doy=yday(date), data=stem/100*yield_bot/10*0.45, sd=5, type="sd",
                                 weight=ifelse(((date>=calib_start)&(date<=calib_end)), 1, 0)) %>% drop_na())
...
data_c[[6]] <- with(temp, tibble(var='WCL', year=year(date), 
                                 doy=yday(date), data=mean_sm, sd=2.5, type="sd",
                                 weight=1) %>% drop_na())
