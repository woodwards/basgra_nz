...
## SETTINGS FOR THE DIFFERENT CALIBRATION SITES (at least one site)
   ...
   sitenames <- c(sitenames, "Northland+CW")
   sitenumbers <- c(sitenumbers, 1)
   sitesettings_filenames <- c(sitesettings_filenames, paste(scenario, "/initialise_BASGRA_Northland+CW.r", sep=""))
   sitedata_filenames     <- c(sitedata_filenames, paste(scenario, "/data_calibration_Northland_0.txt", sep=""))
   
   sitenames <- c(sitenames, "Northland+CG")
   sitenumbers <- c(sitenumbers, 1)
   sitesettings_filenames <- c(sitesettings_filenames, paste(scenario, "/initialise_BASGRA_Northland+CG.r", sep=""))
   sitedata_filenames     <- c(sitedata_filenames, paste(scenario, "/data_calibration_Northland_0.txt", sep=""))

   sitenames <- c(sitenames, "Northland+CT")
   sitenumbers <- c(sitenumbers, 1)
   sitesettings_filenames <- c(sitesettings_filenames, paste(scenario, "/initialise_BASGRA_Northland+CT.r", sep=""))
   sitedata_filenames     <- c(sitedata_filenames, paste(scenario, "/data_calibration_Northland_0.txt", sep=""))
