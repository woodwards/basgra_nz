...
## SETTINGS FOR THE DIFFERENT CALIBRATION SITES (at least one site)
   ...
   sitenames <- c(sitenames, "Northland-10%")
   sitenumbers <- c(sitenumbers, 1)
   sitesettings_filenames <- c(sitesettings_filenames, paste(scenario, "/initialise_BASGRA_Northland-10.r", sep=""))
   sitedata_filenames     <- c(sitedata_filenames, paste(scenario, "/data_calibration_Northland_0.txt", sep=""))
   
   sitenames <- c(sitenames, "Northland-20%")
   sitenumbers <- c(sitenumbers, 1)
   sitesettings_filenames <- c(sitesettings_filenames, paste(scenario, "/initialise_BASGRA_Northland-20.r", sep=""))
   sitedata_filenames     <- c(sitedata_filenames, paste(scenario, "/data_calibration_Northland_0.txt", sep=""))

   sitenames <- c(sitenames, "Northland-30%")
   sitenumbers <- c(sitenumbers, 1)
   sitesettings_filenames <- c(sitesettings_filenames, paste(scenario, "/initialise_BASGRA_Northland-30.r", sep=""))
   sitedata_filenames     <- c(sitedata_filenames, paste(scenario, "/data_calibration_Northland_0.txt", sep=""))
