...
## SETTINGS FOR THE DIFFERENT CALIBRATION SITES (at least one site)
   ...
   sitenames <- c(sitenames, "Waikato+CW")
   sitenumbers <- c(sitenumbers, 2)
   sitesettings_filenames <- c(sitesettings_filenames, paste(scenario, "/initialise_BASGRA_Scott+CW.r", sep=""))
   sitedata_filenames     <- c(sitedata_filenames, paste(scenario, "/data_calibration_Scott_0.txt", sep=""))
   
   sitenames <- c(sitenames, "Waikato+CG")
   sitenumbers <- c(sitenumbers, 2)
   sitesettings_filenames <- c(sitesettings_filenames, paste(scenario, "/initialise_BASGRA_Scott+CG.r", sep=""))
   sitedata_filenames     <- c(sitedata_filenames, paste(scenario, "/data_calibration_Scott_0.txt", sep=""))
   
   sitenames <- c(sitenames, "Waikato+CT")
   sitenumbers <- c(sitenumbers, 2)
   sitesettings_filenames <- c(sitesettings_filenames, paste(scenario, "/initialise_BASGRA_Scott+CT.r", sep=""))
   sitedata_filenames     <- c(sitedata_filenames, paste(scenario, "/data_calibration_Scott_0.txt", sep=""))
