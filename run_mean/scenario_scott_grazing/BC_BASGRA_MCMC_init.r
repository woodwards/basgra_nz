...
## SETTINGS FOR THE DIFFERENT CALIBRATION SITES (at least one site)
   ...
   sitenames <- c(sitenames, "Waikato-10%")
   sitenumbers <- c(sitenumbers, 2)
   sitesettings_filenames <- c(sitesettings_filenames, paste(scenario, "/initialise_BASGRA_Scott-10.r", sep=""))
   sitedata_filenames     <- c(sitedata_filenames, paste(scenario, "/data_calibration_Scott_0.txt", sep=""))
   
   sitenames <- c(sitenames, "Waikato-20%")
   sitenumbers <- c(sitenumbers, 2)
   sitesettings_filenames <- c(sitesettings_filenames, paste(scenario, "/initialise_BASGRA_Scott-20.r", sep=""))
   sitedata_filenames     <- c(sitedata_filenames, paste(scenario, "/data_calibration_Scott_0.txt", sep=""))
   
   sitenames <- c(sitenames, "Waikato-30%")
   sitenumbers <- c(sitenumbers, 2)
   sitesettings_filenames <- c(sitesettings_filenames, paste(scenario, "/initialise_BASGRA_Scott-30.r", sep=""))
   sitedata_filenames     <- c(sitedata_filenames, paste(scenario, "/data_calibration_Scott_0.txt", sep=""))
