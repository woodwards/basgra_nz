...
## SETTINGS FOR THE DIFFERENT CALIBRATION SITES (at least one site)
   ...
   sitesettings_filenames <- c(sitesettings_filenames,
                               paste(scenario, "/initialise_BASGRA_Scott_water.r", sep=""))
   sitedata_filenames     <- c(sitedata_filenames,
                               paste(scenario, "/data_calibration_Scott_0.txt", sep=""))
   
   sitenames <- c(sitenames, "Waikato+CG")
   sitesettings_filenames <- c(sitesettings_filenames,
                               paste(scenario, "/initialise_BASGRA_Scott_grazing.r", sep=""))
   sitedata_filenames     <- c(sitedata_filenames,
                               paste(scenario, "/data_calibration_Scott_0.txt", sep=""))
   
   sitenames <- c(sitenames, "Waikato+CT")
   sitesettings_filenames <- c(sitesettings_filenames,
                               paste(scenario, "/initialise_BASGRA_Scott_temp.r", sep=""))
   sitedata_filenames     <- c(sitedata_filenames,
                               paste(scenario, "/data_calibration_Scott_0.txt", sep=""))
