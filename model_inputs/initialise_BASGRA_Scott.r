...
# dyn.load("BASGRA_WG.DLL") # use this one to calculate PET
...
## 2. SITE CONDITIONS ##
   ...
   file_weather   <- 'weather/weather_Scott.txt'
   file_params    <- 'parameters/parameters_Scott.txt' # can contain multiple columns
     parcol       <- 1 # which one are we going to use? (row names are ignored)
     
   # read harvest days (Simon) which are set up in make_weather1.r   
   days_harvest   <- as.matrix(read.table(file="weather/harvest_Scott.txt", sep='\t'))
