...
# my parameter files
my_par <- arrange(read_tsv("parameters_Scott.txt"), PARAMETER)
my_bc <- arrange(read_tsv("parameters_BC_Scott.txt", skip=1, col_names=FALSE), X1)
names(my_bc) <- c("PARAMETER", "MIN", "MODE", "MAX", "SHAPE", "SITES")

# # check against fortran parameter list (order matters)
# my_par_raw <- read_tsv("parameters_Scott.txt")
...
orig_par <- read_tsv("parameters_Gri_Saerheim.txt", skip=1, col_names=FALSE)
...
orig_bc <- read_tsv("parameters_BC_Gri_Saerheim.txt", col_names=FALSE)
...
new_par <- read_tsv("parameters_Saerheim_nutritive.txt", skip=1, col_names=FALSE)
...
new_bc <- read_tsv("parameters_BC_Saerheim_nutritive.txt", col_names=FALSE)
...
# read old parameters
# file_params    <- 'parameters.txt' # can contain multiple columns
...
all <- select(orig_par, PARAMETER, origMINSITE=MINSITE, origMAXSITE=MAXSITE) %>%
  ...
  arrange(PARAMETER)
# write_tsv(all, "check_variables.tsv")  
...
all %>% kable() %>% 
  ...
  add_header_above(header=c("Parameter" = 1, "Original" = 5, "Nutritive" = 5, "Scott" = 7), 
                   aligh="l", bold=TRUE)
