...
# summary
all <- select(arrange(orig_par, PARAMETER), PARAMETER, origMINSITE=MINSITE, origMAXSITE=MAXSITE) %>%
  left_join(select(orig_bc, PARAMETER, origMIN=MIN, origMODE=MODE, origMAX=MAX)) %>%
  full_join(select(arrange(new_par, PARAMETER), PARAMETER, newMINSITE=MINSITE, newMAXSITE=MAXSITE)) %>%
  left_join(select(new_bc, PARAMETER, newMIN=MIN, newMODE=MODE, newMAX=MAX)) %>%
  full_join(select(arrange(my_par, PARAMETER), PARAMETER, ScottSITE=Scott)) %>%
...
all_formatted %>%
  ...
  add_header_above(header=c("Parameter" = 1,
                            "Original" = 5, "Nutritive" = 5, "Scott" = 5, "Details"=2),
