## 1. INITIALISE MCMC ##
  ...
  # initialise BC
  source('scripts/BC_BASGRA_MCMC_init_Lincoln.r')
  # which in turn calls:
...
## 3. PRINTING AND PLOTTING ##
  ...
  for (s in 1:nSites){
    cat(file=stderr(), paste('Running BASGRA_Lincoln with BC MAP parameters, site',s), "\n")
  ...
  # finish
  cat(file=stderr(), 'Finished BC_BASGRA_Lincoln.r', "\n")
