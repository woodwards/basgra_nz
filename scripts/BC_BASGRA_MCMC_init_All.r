## BC_BASGRA_MCMC_init_xxx.R ##
   cat(file=stderr(), 'Starting BC_BASGRA_MCMC_init_All.r', "\n")

## MCMC chain length (total number of iterations across all chains)
   nChains       <- 3 # chains for repeatability testing
   nBurnin       <- as.integer(12000 * 10 * nChains) # need to check convergence
...
## FILE FOR PRIOR PARAMETER DISTRIBUTION
   ...
   file_prior    <- "model_inputs/parameters_BC_All_Nine.txt"
...
## SETTINGS FOR THE DIFFERENT CALIBRATION SITES (at least one site)
   sitesettings_filenames <- c("scripts/initialise_BASGRA_Northland_2.r",
                               ...
                               "scripts/initialise_BASGRA_Lincoln_2.r",
                               "scripts/initialise_BASGRA_Lincoln_3.r",
                               "scripts/initialise_BASGRA_Lincoln_5.r"
                               )
   sitedata_filenames     <- c("model_inputs/data_calibration_Northland_2.txt",
                               ...
                               "model_inputs/data_calibration_Lincoln_2.txt",
...
## GENERAL INITIALISATION FOR MCMC
   ...
   #
   cat(file=stderr(), 'Finished BC_BASGRA_MCMC_init_All.r', "\n")
