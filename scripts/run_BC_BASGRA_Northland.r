## 1. INITIALISE MCMC ##
  ...
  cat(file=stderr(), 'Starting BC_BASGRA_Northland.r', "\n")
  ...
  # initialise BC
  source('scripts/BC_BASGRA_MCMC_init_Northland.r')
  # which in turn calls:
...
## 3. PRINTING AND PLOTTING ##
  ...
  for (s in 1:nSites){
    cat(file=stderr(), paste('Running BASGRA_Northland with BC MAP parameters, site',s), "\n")
  ...
  # finish
  cat(file=stderr(), 'Finished BC_BASGRA_Northland.r', "\n")
