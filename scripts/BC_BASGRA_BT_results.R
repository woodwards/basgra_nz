...
if (TRUE){
  ...
  # "what if" predictions with modified inputs ####
  if (TRUE){
    
    # save model inputs and parameters 
    old_list_params         <- list_params         
    old_list_matrix_weather <- list_matrix_weather
    old_list_days_harvest   <- list_days_harvest  
    old_list_NDAYS          <- list_NDAYS         
    ...
    for (s in 1:nSites){ 
      ...
      # residual analysis not needed
      
      # plot other model outputs
      cat(file=stderr(), 'Plot new model predictions other, site', s, "\n")
    ...
    # restore model inputs and parameters
    list_params         <- old_list_params         
    list_matrix_weather <- old_list_matrix_weather
    list_days_harvest   <- old_list_days_harvest  
    list_NDAYS          <- old_list_NDAYS         
