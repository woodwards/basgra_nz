...
for (s in 1:nSites){
  ...
  leg[1] <- "Current"
  list_output[[1]] <- run_model(y = matrix(0, NDAYS, NOUT))
  ...
  list_output[[2]] <- run_model(y = matrix(0, NDAYS, NOUT))
  days_harvest <- list_days_harvest[[s]]
  ...
  list_output[[3]] <- run_model(y = matrix(0, NDAYS, NOUT))
  days_harvest <- list_days_harvest[[s]]
  ...
  list_output[[4]] <- run_model(y = matrix(0, NDAYS, NOUT))
  days_harvest <- list_days_harvest[[s]]
  ...
  list_output[[5]] <- run_model(y = matrix(0, NDAYS, NOUT))
  days_harvest <- list_days_harvest[[s]]
